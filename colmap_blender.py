...
def pipeline(scene, base_path, n_views):
    ...
    os.mkdir('images')
    # os.system('colmap model_converter  --input_path ./created/sparse/ --output_path ./created/sparse/  --output_type TXT')


    images = {}
    with open('../created/sparse/images.txt', "r") as fid:
        while True:
            line = fid.readline()
            if not line:
                break
    ...
    img_list = sorted(images.keys(), key=sort_key)
    ...
    if n_views > 0:
        ...
        train_img_list = [c for idx, c in enumerate(img_list) if idx in [2, 16, 26, 55, 73, 76, 86, 93]]

    for img_name in train_img_list:
        os.system('cp ../train/' + img_name + '  images/' + img_name)

    os.system('cp ../created/sparse/cameras.txt created/.')

    with open('created/points3D.txt', "w") as fid:
        pass

    res = os.popen('colmap feature_extractor --database_path database.db --image_path images  --SiftExtraction.max_image_size 800 --SiftExtraction.max_num_features 8192 --SiftExtraction.estimate_affine_shape 1 --SiftExtraction.domain_size_pooling 1').read()
...
for scene in [ 'chair']: # 'chair', 'drums', 'ficus', 'hotdog',  'lego', 'materials', 'mic', 'ship'
    pipeline(scene, base_path = '/media/pc/D/zzt/DNGaussian/DNGaussian0926/data/nerf_synthetic/', n_views = 8)  # please use absolute path!