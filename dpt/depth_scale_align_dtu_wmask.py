...
def data_process(depth_gt,depth_estimate,mask_image):
    ...
    print("depth_estimate", depth_estimate.shape)
    ...
    print('The range of depth before and after 99 filtering',[np.min(depth_estimate),np.max(depth_estimate)],[depth_estimate_min,depth_estimate_max])
    ...
    for k in range(len(depth_estimate_range)-1):
        ...
        if len(scale_list)!=0:
            ...
            # a,b,c=dbscan(torch.tensor(scale_list), eps=0.1) 
    ...
    print(f'final_result:{len(final_result)}')
    ...
    for i in range(depth_estimate.shape[0]):
        for j in range(depth_estimate.shape[1]):
            if depth_estimate[i][j]!=0:
                scale_arr[i][j]=model.predict(depth_estimate[i][j].reshape(1,-1))

    for i in range(depth_estimate.shape[0]):
        for j in range(depth_estimate.shape[1]):
            if depth_estimate[i][j]!=0:
                depth_estimate_save[i][j]=depth_estimate[i][j]*scale_arr[i][j]
    ...
    mse = np.mean((depth_estimate_save - depth_gt) ** 2)
    print('The mse after scaling', mse) 
    if mse>1:
        with open('zzh_abnormal.txt', 'a') as file:
                file.write(str(mse)+file_now+'\n')
                                
    np.save(output_dir+dir_now+'/npy_depth/'+file_now, depth_estimate_save)
...
def main():
    ...
    for dir_name in os.listdir(input_dir):
        if dir_name.startswith("scan") and dir_name == args.scan: # or dir_name=="scan103" and (dir_name=="scan40" )
            ...
            os.makedirs(output_dir+dir_name+'/npy_depth', exist_ok=True)  
            dir_now=dir_name
